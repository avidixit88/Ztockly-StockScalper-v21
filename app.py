...
with col_a:
    scan_now = st.button("Scan Watchlist", use_container_width=True)
...
with col_d:
    auto_refresh = st.toggle("Auto-refresh scan", value=False, help="Re-run scan on a timer while this page is open.")
    refresh_seconds = st.slider("Refresh seconds", 10, 120, 30, 5, help="Only used when Auto-refresh is ON.")
    results = []
    if auto_refresh:
        results = run_scan()
